...
class ascii_by_pca_L1:
    ...
    def patch2ascii(self, img, n=5):
        ...
        for i in range(limit):
            l2 = self.l1_norm(patch_pca, ascii_dataset[i][1])
            if l2 < min_l2:
                min_l2 = l2
                min_l2_position = i
    ...
    def img2ascii_file(self, img = None):
        if img is not None:
            self.img_path = img
            self.img = cv.imread(self.img_path,cv.IMREAD_GRAYSCALE)
            x = img.rsplit("/",1)[-1]
        ...
        file_out_path = f'{self.output_name}.txt'
        print(f"name= {file_out_path}")
        self.output_file = file_out_path
        with open(f'./text/{file_out_path}', 'a') as file:
    ...
    def text_to_image(self, text_lines=None, image_path=None, font_path=None, font_size=0, image_size=None):
        if text_lines == None:
            text_lines = self.load_text(f'./text/{self.output_file}')

        if image_path == None:
            path_name = self.output_file.rsplit(".",1)[0]
            font_name = self.style
            image_path = f'./ascii_frames/{path_name}.png'
...
# obj1.data_prep(img_path, 160, 3)
# obj1.img2ascii_file()
# obj1.text_to_image(font_path = "path/to/cour.ttf")